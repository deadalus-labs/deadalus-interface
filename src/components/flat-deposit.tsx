  ...
  return (
    <>
      <Card className="flex flex-col w-full md:w-[80%] sm:mx-auto text-white bg-[#111827A6]/65 py-5 px-5 sm:px-10 space-y-10 border-0">
        ...
        <CardContent className="flex border-2 border-slate-800 rounded-lg pt-6 text-slate-300 text-lg sm:text-xl justify-center">
          <span>You are creating <span className="font-bold text-white">{frequency === "daily"
            ? "2 tokens"
            : frequency === "weekly"
              ? "7 tokens"
              : "12 tokens"}</span> <span>{frequency === "daily"
                ? ", the first for first of the minute and the other one for the second half"
                : frequency === "weekly"
                  ? ", one for each day of the week"
                  : ", one for each month"}</span></span>
        </CardContent>
        <div className="flex">
          <Button
